...
export function createOffer(
  ...
  debug('creating route for createOffer at ', createOfferPath)
  verifier.router!.post(createOfferPath, passport.authenticate(verifier.name + '-admin', { session: false }), async (request: any, response: any) => {
    try {
      ...
      debug(`presentation == ${rp.presentation}`)
      if (!rp) {
